...
\proj (\textbf{A}utonomous \textbf{I}maging and \textbf{L}earning \textbf{A}i \textbf{RO}bot identifying pla\textbf{N}kton taxa in-situ),  an open source AUV control system with a framework
...
\paragraph{Imaging Pipeline}
In \proj the novel SILCAM has been implemented into the hull of a AUV.
...
\begin{figure}[tbp]
  ...
  \caption{The Light Autonomous Underwater Vehicle (LAUV) Roald, used in
    \proje. The sample volume of the SILCAM, capable of capturing images of microbial biology, can be seen on the underside of the nose. The equipped Doppler Velocity Logger include current profiling capabilities used to assimilate a current field during operation.}
  \label{fig:roald}
\end{figure}
